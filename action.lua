...
function on_interact(m, o, type, value)
    if type == INTERACT_DAMAGE or type == INTERACT_BOUNCE_TOP or type == INTERACT_BOUNCE_TOP2 or type == INTERACT_SNUFIT_BULLET or type == INTERACT_UNKNOWN_08 or type == INTERACT_KOOPA or type == INTERACT_HIT_FROM_BELOW then
        if gPlayerSyncTable[m.playerIndex].star then
            m.flags = m.flags & ~MARIO_METAL_CAP
        elseif m.playerIndex == 0 and is_item(get_id_from_behavior(o.behavior)) and o.oObjectOwner then
            local np = network_player_from_global_index(o.oObjectOwner or 0)
            on_pvp_attack(gMarioStates[np.localIndex], m, false, true)
            m.hurtCounter = 0
            if o.coopFlags & COOP_OBJ_FLAG_NETWORK ~= 0 then
                network_send_object(o, true) -- sync interaction
            end
        end
    elseif type == INTERACT_PLAYER and m.action & ACT_FLAG_INTANGIBLE == 0 and m.invincTimer == 0 then
        local sMario = gPlayerSyncTable[m.playerIndex]
        ...
        if m2.action & ACT_FLAG_INTANGIBLE ~= 0 or m2.invincTimer ~= 0 then return end
        if gPlayerSyncTable[m2.playerIndex].star and not sMario.star then
            if take_damage_and_knock_back(m, o) ~= 0 then
                on_pvp_attack(m2, m)
                m.hurtCounter = 0
                m.invincTimer = math.max(m.invincTimer, 10)
            end
            return
        end
    end
